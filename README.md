...
Kubernetes versions 1.33 and below are supported by the metrics agent on AWS cloud service (EKS), Google Cloud Platform (GKE), Azure cloud services (AKS), and Oracle Cloud (OKE).

### OpenShift Versions

OpenShift versions 4.18 to 4.12 are supported by the metrics agent on ROSA.
...
| CLOUDABILITY_UPLOAD_REGION                         | Optional: The region the metrics-agent will upload data to. Default `us-west-2`. Supported values: `us-west-2`, `eu-central-1`, `ap-northeast-1`, `ap-south-1`, `ap-southeast-1`, `ap-southeast-2`, `me-central-1`, `us-gov-west-1` |
