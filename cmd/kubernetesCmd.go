...
func init() {
	...
	kubernetesCmd.PersistentFlags().BoolVar(
		&config.RetrieveStatsContainers,
		"retrieve_stats_containers",
		true,
		"When true, includes stats containers in metrics collection.",
	)
	kubernetesCmd.PersistentFlags().BoolVar(
	...
	_ = viper.BindPFlag("retrieve_stats_containers", kubernetesCmd.PersistentFlags().Lookup("retrieve_stats_containers"))
	...
	config = kubernetes.KubeAgentConfig{
		...
		RetrieveStatsContainers: viper.GetBool("retrieve_stats_containers"),
